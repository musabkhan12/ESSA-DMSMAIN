...
// addyan 17/3/26
  ...
  const [toggleApprover, setToggleApprover] = useState<"Yes" | "No">("No");
  const [loading, setLoading] = useState(false); // addhyan - 17/03/26
  // INIT
  useEffect(() => {
    setLoading(true); // addhyan - 17/03/26
    (async () => {
      const me = await spChild.web.currentUser();
      currentUserEmail.current = me.Email;
      ...
      await loadUsers();
      await loadWorkflow();
      setLoading(false); // addhyan - 17/03/26
    })();
  }, []);
  ...
  const loadUsers = async () => {
    ...
    const u = await spfilepath.web.siteUsers(); -   {/* Addhyan 17/03/26 */}
    setUsers(
...
const handleSave = async () => {
  ...
  return (
    <div className="modal show d-block" style={{background:"rgba(0,0,0,.2)"}}>
      {/* // Addhyan 17/03/26 - Show loading state */}
      ...
      <div className="modal-dialog modal-xl">
        <div className="modal-content p-4">
          ...
          {toggleApprover==="No"?(
            <>
            {/* Addhyan 17/03/26 */}
            <button style={{background:'transparent', border:'none', display:"flex", flexDirection: "row-reverse",  padding: '0'}} className="btn btn-secondary ms-2" onClick={onClose}>
