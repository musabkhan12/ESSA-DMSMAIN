        ...
        <div className="modal-content p-4">
          ...
          {toggleApprover==="No"?(
            <>
              ...
              <button className="btn btn-primary" onClick={()=>setToggleApprover("Yes")}>Add Approval</button>
            </>
          ):(
            <>
              ...
              {rows.map(row=>(
                <div key={row.id} className="d-flex gap-2 mb-2">
                  ...
                  <Select
                    ...
                    options={users}
                    value={row.approvedUserList}
                    onChange={(v:any)=>setRows(r=>r.map(x=>x.id===row.id?{...x,approvedUserList:v}:x))}
                    className="flex-grow-1"
                  />

                  <div>
                    <input type="radio" checked={row.selectionType==="One"}
                      onChange={()=>setRows(r=>r.map(x=>x.id===row.id?{...x,selectionType:"One"}:x))}/> One
                    <input className="ms-2" type="radio" checked={row.selectionType==="All"}
                      onChange={()=>setRows(r=>r.map(x=>x.id===row.id?{...x,selectionType:"All"}:x))}/> All
                  </div>
              ...
              <button type="button" className="btn btn-link"
                onClick={()=>setRows(r=>[...r,{id:r.length,selectionType:"One",approvedUserList:[]}])}>
                + Add Level
              </button>
