...
const handlebulkFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  if (!files || files.length === 0) {
    setIsUploading(false); // validation change aman 13/03/26
  ...
  const siteCollCheck = urlHashCheck.split('/')[0].replace('#', '');
  const rootUrlCheck = `${window.location.origin}/sites/${siteCollCheck}`;
  const rootWebCheck = Web(rootUrlCheck).using(AssignFrom(sp.web as any));
  const listMasterName = `DMS${currentfolderpath.Entity}FileMaster`;
 
  const existingInMaster = await rootWebCheck.lists.getByTitle(listMasterName).items
    .filter(`CurrentFolderPath eq '${currentfolderpath.folderpath}'`)
    .select("FileName")();
 
  const existingNamesSet = new Set(existingInMaster.map(item => item.FileName.toLowerCase()));
 ...
 // Aman 9/04/26 end 
  ...
  if (duplicateFiles.length > 0) {
    combinedMsg += `<p style="color: #d33; font-weight: bold; margin-bottom: 5px;">
    Files with a name that already exists:
    </p>
    <ul style="margin:8px 0 16px 20px; line-height:1.6;">
    ${duplicateFiles.map(f => `<li>${f}</li>`).join('')}
    </ul>`;
  }
  ...
  // --- Combined Validation Summary End aman 16/03/26 ---
 
  // --- VALIDATION END aman 13/03/26 ---
 
  // const validFiles: File[] = []; // Store valid files // aman 13/03/26: Commented out
  ...
  if (validFilesToProcess.length > 0) {
    if (validFilesToProcess.length === 1) {
      const fileList = new DataTransfer();
      validFilesToProcess.forEach((file) => fileList.items.add(file));
      // uploadFile(validFiles[0]); // Upload single file
      bulkUploadFile(fileList.files);
    } else {
      const fileList = new DataTransfer();
    ...
    setIsUploading(false); // aman 13/03/26: Stop loading if no valid files
  }
...
const bulkUploadFile = async (files: FileList) => {
  ...
  try {
    ...
    for (let i = 0; i < files.length; i++) {
      try {
        const file = files[i];
        // --- validation change aman 13/03/26 comment out the rename logic here also
        ...
        //   counter++;
        // }
        const uniqueFileName = file.name; // --- validation change aman 13/03/26
        ...
        //existingFiles.push({ Name: uniqueFileName } as any);   // --- validation change aman 13/03/26
...
const handleSubmit = async (event: any) => {
  ...
  try {
    ...
    // Aman 16/3/26
    // --- NEW VALIDATION START: Check Duplicate in FileMaster List aman 13/03/26---
    const listName = `DMS${currentfolderpath.Entity}FileMaster`;
    const checkDuplicate = await rootWeb.lists.getByTitle(listName).items
      .filter(`FileName eq '${selectedFile.name}' and CurrentFolderPath eq '${currentfolderpath.folderpath}'`)
      .select("Id")();
    ...
    if (checkDuplicate.length > 0) {
      Swal.fire({
        ...
        text: `A file with the name "${selectedFile.name}" already exists in this folder. Duplicate uploads are not allowed.`,
      });
      setIsFinalUploading(false);
      submitBtn.disabled = false;
      submitBtn.innerText = "Submit";
    ...
    // --- UPLOAD TO SUBSITE ---
    // srs 17/3/26 fix space issue in folder path
    const sanitizedFolderPath = currentfolderpath.folderpath.trim().replace(/\/+$/, "");
    // const folder = subsiteWeb.getFolderByServerRelativePath(currentfolderpath.folderpath);
    // srs 17/3/26 fix space issue in folder path
    const folder = subsiteWeb.getFolderByServerRelativePath(sanitizedFolderPath);
    const files = await folder.files();
    ...
    const uploadResult = await folder.files.addChunked(uniqueFileName, selectedFile);
       ...
       // comment - addhyan 25/03/2026
 
const item = await uploadResult.file.getItem();
const itemDatas = await item.select("Id")();
// console.log("Retrieved item data:", itemDatas);
 
// const itemIds = itemDatas.Id;
// console.log("Retrieved item IDs:", itemIds);
 
 
const itemData: number[] = [];
itemData.push(itemDatas.Id);
...
// commented End by addyan 25/3/26

// comment - added addhyan 25/03/2026
...
const handleSubmitBulk = async (event: any) => {
  ...
  try {
    ...
    console.log("Current Path:", documentLibrary);
     ...
     // --- duplicate validation change aman 13/03/26 ---
    const urlHashFinal = window.location.hash;
    const siteCollFinal = urlHashFinal.split('/')[0].replace('#', '');
    const rootUrlFinal = `${window.location.origin}/sites/${siteCollFinal}`;
    const rootWebFinal = Web(rootUrlFinal).using(AssignFrom(sp.web as any));
    const listMasterFinal = `DMS${currentfolderpath.Entity}FileMaster`;
 
    const existingDataFinal = await rootWebFinal.lists.getByTitle(listMasterFinal).items
      .filter(`CurrentFolderPath eq '${currentfolderpath.folderpath}'`)
      .select("FileName")();
   
    const finalExistingNamesSet = new Set(existingDataFinal.map(item => item.FileName.toLowerCase()));
    let skippedFilesFinal: string[] = [];
    // --- duplicate validation change end aman 13/03/26 ---
    ...
    const uploadPromises = uploadedFiles.map(async ({ name, file }) => {
      try {
        //const uniqueFileName = generateUniqueFileName(name);
        // --- duplicate validation change aman 13/03/26 ---
        if (finalExistingNamesSet.has(name.toLowerCase())) {
          skippedFilesFinal.push(name);
          return null;
        }
        const uniqueFileName = name;
        finalExistingNamesSet.add(name.toLowerCase());
        // --- duplicate validation change end aman 13/03/26
        console.log(`Uploading file: ${uniqueFileName}`);
    ...
    const validItems = uploadedItems.filter(item => item !== null);
     // --- duplicate validation change aman 13/03/26 ---
    if (skippedFilesFinal.length > 0) {
      await Swal.fire({
        icon: 'warning',
        title: 'Duplicates Skipped',
        text: `${skippedFilesFinal.length} file(s) already exist in this folder and were not uploaded.`,
      });
    }
    // --- duplicate validation change end aman 13/03/26 ---
    console.log(validItems ,"validItemsvalidItems")
